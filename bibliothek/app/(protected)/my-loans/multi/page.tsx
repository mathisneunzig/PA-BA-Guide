...
const HANDOVER_OPTIONS: { value: HandoverMethod; label: string; icon: React.ReactNode; description: string }[] = [
  ...
  { value: 'DROPOFF',      label: 'Vorbeibringen', icon: <DirectionsWalkIcon fontSize="small" />, description: 'Mathis soll mir das Buch vorbeibringen' },
]

interface BookEntry {
  input: string        // barcode or Regalnummer the user typed
  barcode: string      // resolved 13-digit barcode
  title: string
  author: string
  resolving: boolean
  error: string
...
function MultiLoanForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (errors.length > 0) {
      ...
      setTimeout(() => router.push('/my-loans'), 1200)
    }
  }
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      >
        {submitting
          ? 'Reserviere…'
          : `${books.filter((b) => b.barcode).length} Buch${books.filter((b) => b.barcode).length !== 1 ? 'er' : ''} reservieren`}
      </Button>
    </Box>
  )
}

export default function MultiLoanPage() {
  return (
    <Container maxWidth="sm" sx={{ py: 4 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
        <LibraryBooksIcon sx={{ fontSize: 36, color: 'primary.main' }} />
        <Box>
          <Typography variant="h5">Sammelreservierung</Typography>
          <Typography variant="body2" color="text.secondary">
            Mehrere Bücher gleichzeitig reservieren.{' '}
            <Link href="/books" style={{ color: 'inherit' }}>Zum Katalog</Link>
          </Typography>
        </Box>
      </Box>
